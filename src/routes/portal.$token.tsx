...
function Skeleton() {
  return <div className="min-h-dvh grid place-items-center text-muted-foreground text-sm">Loading…</div>;
}

function PortalPage() {
  const { token } = Route.useParams();
  const fetchShared = useServerFn(getSharedDocument);
  const { data } = useSuspenseQuery({ queryKey: ["shared", token], queryFn: () => fetchShared({ data: { token } }) });
  ...
  const content = (doc.content ?? {}) as {
    ...
    line_items?: { label: string; quantity: number; unit: string; unit_rate: number; amount: number }[];
    terms?: string;
    ...
    <div className="min-h-dvh bg-background">
      <div className="mx-auto max-w-2xl px-5 py-8 sm:py-12">
        <header className="flex items-start justify-between gap-3 mb-6">
          <div>
            <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground font-semibold">{profile?.business_name ?? "Studio"}</p>
            <h1 className="font-display text-3xl sm:text-4xl mt-1 capitalize">{doc.type} {doc.number ? `#${doc.number}` : ""}</h1>
        ...
        <div className="grid sm:grid-cols-2 gap-4 mb-6">
          ...
          <div className="card-soft p-4">
            <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold mb-2">To</p>
            <p className="font-medium text-sm">{doc.client?.name ?? "—"}</p>
            {doc.client?.company && <p className="text-xs text-muted-foreground">{doc.client.company}</p>}
            {doc.client?.email && <p className="text-xs text-muted-foreground">{doc.client.email}</p>}
          </div>
        </div>
        ...
        {(content.sections ?? []).map((s, i) => (
          <div key={i} className="card-soft p-5 mb-4">
            ...
            <p className="text-sm leading-relaxed whitespace-pre-wrap text-foreground/85">{s.body}</p>
          </div>
        ))}

        {(content.line_items ?? []).length > 0 && (
          <div className="card-soft p-5 mb-4">
            <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold mb-3">Line items</p>
            <div className="space-y-2">
              {(content.line_items ?? []).map((li, i) => (
                <div key={i} className="flex items-center justify-between text-sm border-b border-border/60 pb-2 last:border-0">
                  <div className="min-w-0 pr-3">
                    <p className="truncate">{li.label}</p>
                    <p className="text-xs text-muted-foreground">{li.quantity} × {formatCurrency(li.unit_rate, doc.currency)} / {li.unit}</p>
                  </div>
                  <p className="font-medium tabular-nums">{formatCurrency(li.amount, doc.currency)}</p>
                </div>
              ))}
            </div>
            <div className="mt-4 space-y-1 text-sm">
              <Row label="Subtotal" value={formatCurrency(Number(doc.subtotal ?? 0), doc.currency)} />
              <Row label="VAT" value={formatCurrency(Number(doc.tax ?? 0), doc.currency)} />
              <div className="h-px bg-border my-1.5" />
              <Row label="Total" value={formatCurrency(Number(doc.total ?? 0), doc.currency)} bold />
            </div>
          </div>
        )}
        ...
        {content.terms && (
          <div className="card-soft p-5 mb-4">
            <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold mb-2">Terms</p>
        ...
        {content.payment_instructions && (
          <div className="card-soft p-5 mb-4">
            <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold mb-2">Payment</p>
            <p className="text-sm whitespace-pre-wrap text-foreground/85">{content.payment_instructions}</p>
          </div>
        )}
...
function Row({ label, value, bold }: { label: string; value: string; bold?: boolean }) {
  return (
    <div className="flex justify-between">
      ...
      <span className={bold ? "font-display text-primary" : "font-medium tabular-nums"}>{value}</span>
    </div>
  );
}
